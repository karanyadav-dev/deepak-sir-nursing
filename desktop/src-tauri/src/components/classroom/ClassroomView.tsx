...
import {
  ArrowLeft, Presentation, FileText, Box, Video, PenTool,
  Save, Undo, Redo, Trash2, Download, Maximize,
  Type, Bold, Italic, Underline, Palette, Highlighter,
  Eraser, MousePointer2, Circle, Square, Minus, ArrowRight,
...
export default function ClassroomView() {
  ...
  const [tool, setTool] = useState<'pen' | 'eraser' | 'text' | 'line' | 'rect' | 'circle' | 'arrow' | 'select'>('pen');
    ...
    <div className="h-screen flex flex-col bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700">
        <div className="flex items-center gap-3">
          <button
            onClick={handleBack}
            className="flex items-center gap-2 px-3 py-1.5 bg-red-600 hover:bg-red-700 rounded-md text-sm font-medium"
          >
            <ArrowLeft size={16} />
            Back
          </button>
          <span className="text-lg font-bold text-blue-400">🎓 Classroom</span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 flex flex-col">
          ...
          {/* SUB-TOOLBAR (whiteboard tools) */}
          {contentType === 'whiteboard' && (
            <div className="flex items-center gap-2 px-4 py-2 bg-gray-750 border-b border-gray-700 overflow-x-auto">
              {/* Tools */}
              <button
              ...
              <button
                onClick={() => { setTool('pen'); setHighlighter(false); }}
              ...
              <button
                onClick={() => { setTool('highlighter'); setHighlighter(true); }}
              ...
              <button
                ...
                className={`p-2 rounded ${tool === 'rect' ? 'bg-blue-600' : 'bg-gray-700'}`}
                title="Rectangle"
              >
                <Square size={16} />
              </button>
              <button
                ...
                className={`p-2 rounded ${tool === 'circle' ? 'bg-blue-600' : 'bg-gray-700'}`}
                title="Circle"
              >
                <Circle size={16} />
              </button>
              <button
                ...
                className={`p-2 rounded ${tool === 'arrow' ? 'bg-blue-600' : 'bg-gray-700'}`}
                title="Arrow"
              >
                <ArrowRight size={16} />
              </button>
          ...
          <div className="flex-1 flex items-center justify-center overflow-auto p-4 bg-gray-900">
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="w-[800px] h-[450px] bg-white rounded-lg flex items-center justify-center text-gray-800 shadow-2xl">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 mt-2">PPT Viewer</p>
                  </div>
                </div>
            ...
            {contentType === 'pdf' && (
              <div className="flex flex-col items-center gap-4">
                <div
                  className="w-[800px] h-[1000px] bg-white rounded-lg shadow-2xl flex items-center justify-center text-gray-800"
                  style={{ transform: `scale(${zoom / 100})` }}
                >
                  <div className="text-center">
                    <FileText size={64} className="mx-auto mb-4 text-red-500" />
                    <p className="text-2xl font-bold">Page {currentSlide}</p>
                    <p className="text-gray-500 mt-2">PDF Viewer</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={handleZoomOut} className="px-3 py-2 bg-gray-700 rounded-md">
                    <ZoomOut size={16} />
                  </button>
                  <span className="text-sm">{zoom}%</span>
                  <button onClick={handleZoomIn} className="px-3 py-2 bg-gray-700 rounded-md">
                    <ZoomIn size={16} />
                  </button>
                  <button onClick={handlePrev} className="px-4 py-2 bg-blue-600 rounded-md">
                    ← Prev
                  </button>
                  <span className="text-sm">Page {currentSlide}</span>
                  <button onClick={handleNext} className="px-4 py-2 bg-blue-600 rounded-md">
                    Next →
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="w-[600px] h-[500px] bg-gradient-to-br from-blue-900 to-purple-900 rounded-lg shadow-2xl flex items-center justify-center">
                  ...
                  >
                    ...
                    <p className="text-xl font-bold">3D Heart Model</p>
                    <p className="text-gray-400 text-sm mt-2">Use controls below to rotate</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button
                  ...
                  >
                    <RotateCw size={16} className="inline mr-1" /> Rotate Left
                  </button>
                  <button onClick={handleZoomOut} className="px-3 py-2 bg-gray-700 rounded-md">
                    <ZoomOut size={16} />
                  </button>
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-purple-600 rounded-md"
                  >
                    Rotate Right <RotateCw size={16} className="inline ml-1" />
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="w-[800px] h-[450px] bg-black rounded-lg shadow-2xl flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-2">Blood Flow Animation.mp4</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button
          ...
          <div className="w-80 bg-gray-800 border-l border-gray-700 flex flex-col">
            ...
            <div className="p-3 border-t border-gray-700">
              <button
                ...
                className="w-full px-4 py-2 bg-green-600 rounded-md hover:bg-green-700 flex items-center justify-center gap-2"
              >
